...
import { InputConfig, Subscribable, PromClient } from './types';
...
export class Subscriber {
  ...
  private async _handleNewHeadSubscriptions(): Promise<void> {
    this.api.rpc.chain.subscribeNewHeads(async (header) => {
      this._producerHandler(header as any);
      this._validatorStatusHandler();
      this._payeeChangeHandler(header as any);
      this._commissionChangeHandler(header as any);
  ...
  private async _checkUnexpected(): Promise<void> {
    ...
    this.validators.forEach((v) => {
      ...
      if (isOkCommission) this.promClient.resetStatusValidatorCommissionUnexpected(v.name, v.address);
      else {
        this.logger.info(
          `Detected Unexpected commission for validator ${v.name}: expected percentage ${v.expected.commission}, actual in ppb ${actualCommission}`
        );
      ...
      if (v.expected?.payee) {
        ...
        switch (v.expected.payee) {
          ...
          default:
            isOkPayee =
              actualRewardDestination?.isAccount && v.expected.payee == actualRewardDestination?.asAccount.toString();
            break;
        }
      }
      if (isOkPayee) {
        ...
        this.logger.info(
          `Detected Unexpected payee for validator ${v.name}: expected ${v.expected.payee}, actual ${JSON.stringify(
            actualRewardDestination
          )}`
        );
  ...
  private async _validatorStatusHandler(): Promise<void> {
    this.validators.forEach(async (account) => {
      ...
      if (validatorActiveSetIndex < 0) {
        this.logger.debug(
          `Target ${account.name} is not present in the validation active set of era ${this.currentEraIndex}`
        );
  ...
  private _handlePayeeChangeDetection(signer: { toString(): string }): void {
    for (const validator of this.validators) {
  ...
  private _handleCommissionChangeDetection(signer: { toString(): string }): void {
    for (const validator of this.validators) {
      if (signer.toString() == validator.address || signer.toString() == validator.controllerAddress) {
  ...
  private _slashedEventHandler(event: { data: any[] }): void {
